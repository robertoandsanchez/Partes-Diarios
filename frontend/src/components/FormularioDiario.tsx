...
export function FormularioDiario({ idEdicion, alTerminar }: { idEdicion: number | null, alTerminar: () => void }) {
  ...
  const qOperarios = useQuery({ 
    ...
    queryFn: () => {
      ...
      return fetcher(`/operarios?sectorId=${sectorActual}`);
    },
  ...
  return (
    <div className="space-y-6 pb-20 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-800 pb-6">
        ...
        {ultimoId && ( <button onClick={descargarPDF} className="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-medium border border-green-500 flex items-center gap-2 animate-pulse"><FileDown size={16} /> Descargar PDF</button> )}
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-[#161b22] border border-gray-800 rounded-xl p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-4 text-blue-400">
            <Briefcase size={20} /> <h3 className="font-semibold text-white">Información</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="md:col-span-2">
                <label className="text-xs font-semibold text-blue-400 uppercase tracking-wider flex items-center gap-1.5 mb-1.5">
                    Sector (Filtra Operarios)
                </label>
        ...
        <div className="bg-[#161b22] border border-gray-800 rounded-xl p-6 shadow-sm">
          ...
          {!sectorActual && (
            <div className="bg-yellow-900/20 border border-yellow-700/50 text-yellow-200 p-4 rounded-lg mb-4 flex items-center gap-3 text-sm">
                <AlertTriangle size={20}/> Primero selecciona un Sector arriba para ver la lista de operarios disponibles.
            </div>
          )}
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-800">
            <table className="w-full text-left text-sm text-gray-400">
              <thead className="bg-[#0d1117] text-gray-200 uppercase text-xs font-semibold">
                <tr><th className="px-4 py-3">Operario</th><th className="px-4 py-3">Actividad</th><th className="px-4 py-3 w-24 text-center">Horas</th><th className="px-4 py-3 w-16"></th></tr>
              </thead>
              <tbody className="divide-y divide-gray-800">
                {fields.map((field, index) => (
                  <tr key={field.id} className="hover:bg-gray-800/50">
                    <td className="p-2">
                        <select 
                            {...register(`detalles.${index}.operarioId` as const, { required: true })} 
                            className="input-table"
                            disabled={!sectorActual}
                        >
                            <option value="">{sectorActual ? "Seleccionar..." : "Sin Sector"}</option>
                            {qOperarios.data?.map((i: any) => <option key={i.id} value={i.id}>{i.nombre}</option>)}
                        </select>
                    </td>
      ...
      <style>{`
        .input-pro { width: 100%; background: #0d1117; border: 1px solid #30363d; border-radius: 0.5rem; padding: 0.6rem 1rem; color: white; transition: all 0.2s; }
        .input-pro:focus { border-color: #3b82f6; outline: none; }
        .input-table { width: 100%; background: transparent; padding: 0.5rem; color: #e5e7eb; border-radius: 0.375rem; }
        .input-table:focus { background: #161b22; outline: none; box-shadow: 0 0 0 1px #3b82f6; }
        .btn-secondary { background: #30363d; color: white; padding: 0.4rem 0.8rem; border-radius: 0.5rem; display: flex; align-items: center; gap: 0.5rem; }
